...
class Player:
    ...
    def choose_interaction(self, agents_dict):
        ...
        print(
            "Игрок может взаимодействовать с агентами, которые к нему подошли "
            "на основе их отношений."
        )
    ...
    def respond_to_agent(self, agent_name, emotion_name, emotion_value):
        """Обрабатывает ответ игрока на эмоцию другого агента."""
        print(
            f"{self.name} отвечает на эмоцию {emotion_name} с силой {emotion_value} "
            f"агента {agent_name}."
        )
