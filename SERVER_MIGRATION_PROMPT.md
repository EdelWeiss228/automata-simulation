...
> 
> **Твоя текущая задача:** Мы запускаем симуляцию на сервере с ClickHouse. Я только что поднял докер, и нам нужно создать класс `ClickHouseLogger` (аналог старого CSVLogger в Python) и асинхронный фоновый воркер-коннектор для Bulk Insert. Используй пакет `clickhouse-connect` в Python. Жду от тебя план.
